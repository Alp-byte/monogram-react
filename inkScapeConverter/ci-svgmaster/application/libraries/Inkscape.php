...
define( 'INKSCAPE_PATH', '"C:\Program Files"\inkscape\\inkscape.exe' );
define( 'PSTOEDIT_PATH', '"C:\Program Files"\pstoedit\\pstoedit.exe' );
...
class Inkscape
{
    ...
    {
        $this->addParam( 'export-' . 'eps', $filename.'.eps' );
        $this->execute();

        if ( ! file_exists( $filename.'.eps' ) )
        {
            show_error('Conversion to Eps Fail', 500, $heading = 'An Error Was Encountered');
            return false;
        }

        if ($format == 'dxf')
        {
            if ( defined( 'PSTOEDIT_PATH' ) )
            {
                $exec = PSTOEDIT_PATH;
            }
            else
            ...
            $res = shell_exec($exec . ' -ndt -f dxf_s:-polyaslines ' . $filename . '.eps ' . $filename . '.dxf');

            if ( ! file_exists( $filename.'.dxf' ) )
            {
                show_error('Conversion to Dxf Fail', 500, $heading = 'An Error Was Encountered');
                return false;
            }
        }
